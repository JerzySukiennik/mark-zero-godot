...
## `reach` is how close it tries to get, in metres. `web` is how many web hits it take to
## glue — the whole point of the big ones. `notice` is how far off he reacts to you at all,
## and `nerve` is how likely he is to stand his ground rather than back off or run.
##
## ORDINARY MEN DIE EASILY. Jurek: "musi być tak, że prosto ich zabić, niektórych. Oprócz
## tych dużych." A thug at 16 hp is two repulsor bolts or one good punch; the brute is
## twelve bolts and is the only thing here you have to commit to.
static var KINDS := {
	# Fists. Has to be right on top of you, hits hard when it gets there, and dies fast.
	"brawler": {
		name = "THUG", hp = 16.0, speed = 6.4, reach = 2.0, web = 1,
		damage = 9.0, rate = 0.85, ranged = false, scale = 1.0,
		notice = 26.0, nerve = 0.55,
	...
	"brute": {
		name = "BRUTE", hp = 190.0, speed = 4.8, reach = 3.0, web = 4,
		damage = 22.0, rate = 1.5, ranged = false, scale = 1.45,
		notice = 40.0, nerve = 1.0,
...
static func spec(kind: String) -> Dictionary:
	var base := {
		...
		spread = 0.02, burst = 1, rocket = false, blast = 0.0, scale = 1.0,
		web_decay = 1.0, knockback_resist = 0.0,
		notice = 30.0, nerve = 0.5, volley = 5.0,
